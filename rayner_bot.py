...
if STREAMLIT_AVAILABLE:
    def run_ui():
        ...
        # Pop-up high R:R alerts
        alerts = scan_high_rr_markets(account_balance)
        for alert in alerts:
            st.toast(f"🚨 {alert['market']} signal `{alert['signal']}` R:R {alert['rr']} at {alert['confidence']}% confidence", icon="⚡")
