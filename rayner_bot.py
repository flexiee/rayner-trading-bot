# app.py
# Pro Trading Bot — real-time Binance public data, multi-TF confirmation, 1:3 RR,
# backtest, scanner, telegram alerts, and signal history.
#
# Copy this file to your repo and run:
...
# ---------- Utility: fetch klines from Binance public REST ----------
...
def pip_size_for_market(market_type: str, symbol_label: str):
    # For crypto use absolute units; for forex this would differ
...
def position_size(account_balance: float, risk_pct: float, entry: float, sl: float, market_type: str):
    ...
    pip_distance = abs(entry - sl) / pip if pip else 0
    if pip_distance <= 0:
        return 0.0, "lots", round(risk_amount, 2)
...
# ---------- Core multi-TF scoring & signal generation ----------
def score_and_generate(df1m: pd.DataFrame, market_label: str, cfg=DEFAULTS):
    ...
    # Build 5m & 15m MTFs if enough data
    ...
    # compute SL/TP as 1 ATR and 3x reward
...
# ---------- Backtest (simple) ----------
def backtest_df(df, market_label, cfg=DEFAULTS, max_hold_bars=240):
    results = []
    n = len(df)
    if n < 200:
...
# ---------- UI ----------
st.title("🚀 Pro Trading Bot — Real-time (Binance public)")
...
with col_left:
    ...
    if os.path.exists(HISTORY_FILE):
        ...
        st.dataframe(hist.sort_values("timestamp", ascending=False).head(20))
    else:
        st.write("No history yet.")

with col_right:
    st.subheader("Chart (visual) & Generate")
    # TradingView embed: use Binance symbol for display
    tv_sym = MARKETS[market_label]['sym']
    st.components.v1.html(f'<iframe src="https://s.tradingview.com/widgetembed/?symbol=BINANCE:{tv_sym}&interval={interval_choice.replace(\"m\",\"\")}&hidesidetoolbar=1&symboledit=1&hideideas=1&theme=dark" width="100%" height="520" frameborder="0"></iframe>', height=540)
    st.markdown("---")

    if st.button("🔮 Generate Signal"):
        with st.spinner("Fetching data and analyzing..."):
            # fetch 1m bars always to allow resample
            df1m = fetch_klines_binance(MARKETS[market_label]['sym'], interval="1m", limit=1500)
            if df1m is None or df1m.empty:
                st.error("Failed to fetch market data.")
            else:
                # If user chose 5m/15m entry we still base calculations on 1m but can resample for confirmation
                ...
                if result['signal'] == 'NONE' or result['confidence'] < DEFAULTS['min_confidence']:
                    st.warning(f"No strong signal. Confidence: {result['confidence']}. Strength: {result['strength']}")
...
# ---------- Backtest & Scanner controls ----------
st.markdown("---")
st.subheader("Backtest & Market Scanner")

col_a, col_b = st.columns(2)
with col_a:
    if st.button("Run Backtest (90d)"):
        st.info("Running backtest (this may take ~1-3 mins depending on data) ...")
        df_bt = fetch_klines_binance(MARKETS[market_label]['sym'], interval="1m", limit=90*24*60)  # approx 90 days 1m
...
with col_b:
    if st.button("Scan Markets (quick)"):
        ...
        for mk in MARKETS.keys():
            ...
            results.append({"market": mk, "signal": r['signal'], "confidence": r['confidence'], "strength": r['strength'], "entry": r['entry'], "sl": r['sl'], "tp": r['tp']})
            time.sleep(0.2)
        if not results:
            st.warning("No scan results (data issue).")
        else:
            dfscan = pd.DataFrame(results).sort_values(["confidence","strength"], ascending=False)
            st.dataframe(dfscan)
            if tg_token and tg_chat:
                top = dfscan[dfscan['signal']!='NONE'].head(3)
                for _, row in top.iterrows():
                    text = f"Scanner: {row['signal']} {row['market']} entry:{row['entry']} sl:{row['sl']} tp:{row['tp']} conf:{row['confidence']}"
                    send_telegram(tg_token, tg_chat, text)
                st.info("Top scanner signals sent to Telegram (if credentials provided).")
...
st.caption("Educational tool. Paper-test before using real money. Not financial advice.")
